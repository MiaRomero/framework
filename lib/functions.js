...
myFunctions.readFile  = function(pathName, fileName, routes) {
  routes.get(pathName, function (req, res) {
    var data = fs.readFileSync(__dirname + '/..' + pathName + '/' + fileName);
    console.log('The below is the DATA contained in the file ' + fileName + ':')
...
myFunctions.fileNamerTimeDTS = function() {
...
// a backslash '/'. IE: /test
myFunctions.directoryListAllFiles = function(pathName, routes) {
  routes.get('/listfiles', function (req, res) {
    fs.readdir(__dirname + '/..' + pathName, (err, files) => {
      if(!files.length) {
...
myFunctions.postJSONtoFILE = function(pathName, routes, fileName) {
  routes.post(pathName, function(req, res) {
    req.on('data', (data) => {
      var postFileName = fileName || myFunctions.fileNamerTimeDTS();
      console.log('saving JSON data to /test');
      fs.writeFile(__dirname + '/..' + pathName + postFileName + '.json',data, (err) => {
        if(err) console.log(err);
        res.writeHead(200, {
          'Content-Type': 'application/json'
        });
        console.log('JSON data input saved to ' + pathName + postFileName + '.json');
        res.write('{"message":"saving JSON data to ' + pathName + postFileName + '"}');
        res.end();
      });
    });
  });
...
// it will run stats on the pathName instead.
myFunctions.getFileInfo = function(pathName, routes, fileName) {
